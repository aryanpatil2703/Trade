...
export default function NetworkSwitcher() {
  ...
  const currentChain = SUPPORTED_CHAINS.find(c => c.id === (chain?.id || currentChainId));
  const isSupportedChain = currentChain !== undefined;

  const handleSwitchChain = async (chainId: number) => {
    try {
      // Use window.ethereum to switch networks
    ...
    <div className="relative">
      ...
      >
        ...
        <span className="text-sm font-medium">
          {currentChain?.name || `Chain ${chain?.id || currentChainId}`}
        </span>
        <svg 
              ...
              {SUPPORTED_CHAINS.map((chainConfig) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded-lg transition-colors ${
                    (chain?.id || currentChainId) === chainConfig.id
